...
LDFLAGS := -ldflags "-s -w \
           -X '$(MODULE_PATH)/cmd/internal/version.Version=$(VERSION)' \
           -X '$(MODULE_PATH)/cmd/internal/version.CommitHash=$(COMMIT)' \
           -X '$(MODULE_PATH)/cmd/internal/version.BuildDate=$(DATE)'"
